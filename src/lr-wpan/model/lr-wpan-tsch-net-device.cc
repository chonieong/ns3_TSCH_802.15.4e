...
#include "lr-wpan-tsch-mac.h" //change by ding
...
LrWpanTschNetDevice::CompleteConfig (void)
{
  ...
  m_mac->SetPhy (m_phy);
  m_omac->SetPhy (m_phy);
  m_omac->SetCsmaCa (m_csmaca); //original is m_omac by ding
  m_mac->SetCsmaCa (m_csmaca); //add by ding
  ...
  m_csmaca->SetMac (m_omac); //original m_omac by ding
  // m_csmaca->SetMac (m_mac); //add by ding //if command than lr-wpan-tsch-test can run     // ppp
  ...
  //change by ding
  // extern bool csmacamode;
  // std::cout<<csmacamode<<std::endl;
  // if(csmacamode)
  // {
  //   m_csmaca->SetLrWpanMacStateCallback (MakeCallback (&LrWpanMac::SetLrWpanMacState, m_omac)); //if command then lr-wpan-tsch-test is without csma, throughput decrease by ding
  // }

  m_csmaca->SetLrWpanMacStateCallback (MakeCallback (&LrWpanMac::SetLrWpanMacState, m_omac)); //if command then lr-wpan-tsch-test is without csma, throughput decrease by ding
  // m_csmaca->SetLrWpanMacStateCallback (MakeCallback (&LrWpanMac::SetLrWpanMacState, m_mac)); //change by ding
...
//maybe can be change here by ding
Address
...
LrWpanTschNetDevice::Send (Ptr<Packet> packet, const Address& dest, uint16_t protocolNumber)
{
  ...
  //std::cout << " In LrWpanTschNetDevice::Send packet : " << packet->GetSize() << std::endl;
  //NS_LOG_DEBUG(" In LrWpanTschNetDevice::Send packet : " << packet->GetSize() );
  ...
  assert(m_isTsch>=0);
  if ( m_isTsch) {
    // NS_LOG_UNCOND("in m_istsch"); //change by ding
    ...
    // NS_LOG_UNCOND(" not in m_istsch"); //change by ding
...
LrWpanTschNetDevice::Send (Ptr<Packet> packet, const Address& dest, bool use_ack, uint16_t protocolNumber)
{
  ...
  assert(m_isTsch>=0);
  if ( m_isTsch) {
    // NS_LOG_UNCOND("in ack m_istsch"); //change by ding
	  TschMcpsDataRequestParams m_mcpsDataRequestParams;
	  LrWpanFrameControlOptions frmcontrol;
	  m_mcpsDataRequestParams.m_frameControlOptions = frmcontrol;
	  m_mcpsDataRequestParams.m_dstAddr = Mac16Address::ConvertFrom (dest);
    ...
    // NS_LOG_UNCOND("not in ack m_istsch"); //change by ding
	  McpsDataRequestParams m_mcpsDataRequestParams;
	  m_mcpsDataRequestParams.m_dstAddr = Mac16Address::ConvertFrom (dest);
	  m_mcpsDataRequestParams.m_dstAddrMode = SHORT_ADDR;
	  m_mcpsDataRequestParams.m_dstPanId = m_omac->GetPanId ();
...
LrWpanTschNetDevice::SetTschMode (bool enable)
{
  ...
  if (m_isTsch!=true && enable) {
	  ...
	  m_phy->SetPlmeCcaConfirmCallback (MakeCallback (&LrWpanTschMac::PlmeCcaConfirm, m_mac)); //change by ding
    // m_phy->SetPlmeCcaConfirmCallback (MakeCallback (&LrWpanCsmaCa::PlmeCcaConfirm, m_csmaca));// change by ding
