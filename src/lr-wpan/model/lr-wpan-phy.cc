...
//const uint32_t LrWpanPhy::aMaxPhyPacketSize = 127; // max PSDU in octets  //default 
const uint32_t LrWpanPhy::aMaxPhyPacketSize = 300; // max PSDU in octets
...
LrWpanPhy::StartRx (Ptr<SpectrumSignalParameters> spectrumRxParams)
{
  ...
  if (m_trxState == IEEE_802_15_4_PHY_RX_ON && !m_setTRXState.IsRunning () && lrWpanRxParams != 0)
      ...
      *interferenceAndNoise += *m_noise;
      ...
      if (LrWpanSignalPower >= m_rxSensitivity)
      ...
      else
        {
          ...
          //std::cout << " Packet discarded due to low energy: " << LrWpanSignalPower << std::endl;
    ...
    {
      // Drop the new packet.
      //std::cout << " packet collision " << std::endl;
  ...
  else
    {
      ...
      //std::cout << " transceiver not in RX state " << std::endl;
  ...
  if (m_trxState == IEEE_802_15_4_PHY_BUSY_RX)
    {
      ...
      if (currentRxParams && !m_currentRxPacket.second)
      {
          ...
          if (m_errorModel != 0)
              ...
              *interferenceAndNoise += *m_noise;
              double sinr = LrWpanSpectrumValueHelper::TotalAvgPower (currentRxParams->psd,m_phyPIBAttributes.phyCurrentChannel)
                ...
                */
                ...
                if (randomValue < per)
                  {
                    //std::cout << " randomValue : " << randomValue << " < " << " per : " << per <<std::endl;
                    //std::cout << " packetType : " << packetType << std::endl;
                    if (packetType == IEEE_802_15_4_PPDU_PAYLOAD)  //2
                    {
                     m_currentRxPacket.second = true;
                     NS_LOG_DEBUG (this << " Packet will be dropped after receiving due to wrong payload");
                     //std::cout << " Packet will be dropped after receiving due to wrong payload " << std::endl;

                    }

                    else if(packetType == IEEE_802_15_4_PPDU_SHR)  //0
                    {
                        ...
                        //std::cout << " Packet dropped due to wrong received preamble " << std::endl;
                    }

                    else if(packetType == IEEE_802_15_4_PPDU_PHR) //1
                    {
                       ...
                       if (payloadLengthSet > payloadLengthCorrect)
                       {
                           ...
                           //std::cout << " Packet receiving energy wasted due to wrong received datalength " << std::endl;
                       ...
                       else if (payloadLengthSet < payloadLengthCorrect)
                       {
                           ...
                           //std::cout << " Packet is cutted and will be discarded at MAC due to wrong received datalength " << std::endl;

                       }
                       else
                       {
                           NS_LOG_DEBUG (this << " Packet is received without effect of wrong received datalength");
                          //std::cout << " Packet is received without effect of wrong received datalength " << std::endl;
            ...
            {
              //std::cout << " Missing ErrorModel " << std::endl;
      ...
      {
        //std::cout << " No Effective Receiving Packet " << std::endl;
  ...
  if (state == IEEE_802_15_4_PHY_TRX_OFF)
    {
      ...
      if ((m_trxState == IEEE_802_15_4_PHY_BUSY_RX) && (m_currentRxPacket.first))
               ...
               || m_trxState == IEEE_802_15_4_PHY_TRX_SWITCHING || m_trxState == IEEE_802_15_4_PHY_TRX_START)
        {
          ...
          //std::cout << " Come in pyh and changing at 876 " << std::endl;
          //delete
  ...
  if (state == IEEE_802_15_4_PHY_FORCE_TRX_OFF)
    {
      ...
      else
        {
          ...
          //std::cout<< " Come in phy and changing at 989 " << std::endl;
          //delete
          ChangeTrxState (IEEE_802_15_4_PHY_TRX_OFF);
                ...
                if (!m_plmeSetTRXStateConfirmCallback.IsNull ())
                  {
                    ...
                    //std::cout<< " Come in phy make IEEE_802_15_4_PHY_TRX_OFF " << std::endl;
                    //delete
...
LrWpanPhy::EndCca (void)
{
  ...
  if (PhyIsBusy ())
    ...
    { //sec 6.9.9 ED detection
      ...
      //std::cout << " In PhyCCAMode 1 " << std::endl;
    ...
    {
      //std::cout << " In PhyCCAMode 2 " << std::endl;

      //sec 6.9.9 carrier sense only
      //CCA Mode 2: Carrier sense only. CCA shall report a busy medium only upon the detection of 
      //a signal compliant with this standard with the same modulation and spreading characteristics 
      //of the PHY that is currently in use by the device. This signal may be above or below the ED threshold.
      if (m_trxState == IEEE_802_15_4_PHY_BUSY_RX)
        {
    ...
    { //sect 6.9.9 both
      ...
      //std::cout << " In PhyCCAMode 3 " << std::endl;
...
LrWpanPhy::HandleEnergyDepletion ()
{
  ...
  //std::cout << "Transceiver is forced to be Off due to Energy Depletion " << std::endl;
  //delete
