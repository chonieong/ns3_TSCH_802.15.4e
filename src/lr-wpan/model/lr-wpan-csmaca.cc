...
{
  NS_LOG_FUNCTION (this);
  // NS_LOG_UNCOND("in lr-wpan-csmaca-start");
  m_NB = 0;
  if (IsSlottedCsmaCa ())
    {
      // NS_LOG_UNCOND("in slotted csmaca");
      m_CW = 2;
      if (m_BLE)
        {
    ...
    {
      // NS_LOG_UNCOND("in unslotted csmaca");
      m_BE = m_macMinBE;
...
{
  NS_LOG_FUNCTION (this);
  // NS_LOG_UNCOND("in set lrwpan mac state call back");
...
{
  // NS_LOG_UNCOND("in get nb "); //change by ding
  // uint8_t backoff=0;
  // backoff++;
