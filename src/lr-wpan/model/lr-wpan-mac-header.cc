...
uint8_t
LrWpanMacHeader::GetNextPacket (void) const   // add by ieong
{
  return(m_nextpacket);
}
...
void
LrWpanMacHeader::SetNextPacket (uint8_t control)   // add by ieong
{
  m_nextpacket = control;
  //std::cout << " adding ping6 in header : " << int(control) << std::endl;
}
...
LrWpanMacHeader::Print (std::ostream &os) const
{
  ...
  if (m_fctrlIEListPresent == 1) {
    ...
    for (std::list<HeaderIE>::const_iterator it = headerie.begin();it!=headerie.end();it++) {
      //std::cout << " Lenght = " << (uint32_t) it->length << std::endl;
...
LrWpanMacHeader::GetSerializedSize (void) const
{
  ...
  // add by ieong
  if( m_fctrlFrmType == LRWPAN_MAC_DATA){ // only add in transmitting data
    size+=1;  // add 1 byte for ping6 
  }
  //size+=1; 
...
LrWpanMacHeader::Serialize (Buffer::Iterator start) const
{
  ...
  if( m_fctrlFrmType == LRWPAN_MAC_DATA){  // only add in transmitting data
    ...
    i.WriteU8 (GetNextPacket());   
    //i.WriteHtolsbU16 (GetNextPacket()); // for 512

    //std::cout << " writing : " << int(GetNextPacket()) << std::endl;
    //i.WriteU8 (m_nextpacket);   
...
LrWpanMacHeader::Deserialize (Buffer::Iterator start)
{
  ...
  // add by ieong
  if( m_fctrlFrmType == LRWPAN_MAC_DATA){     // only add in transmitting data
    SetNextPacket (i.ReadU8 ());  
    //SetNextPacket (i.ReadLsbtohU16 ());  // for 512
    //std::cout << " reading : " << int(GetNextPacket()) << std::endl;
    //m_nextpacket = i.ReadU8 ();
