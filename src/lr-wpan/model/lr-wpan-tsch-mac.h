...
class Packet;
class LrWpanCsmaCa; //not supported at the moment //change by ding
...
struct MlmeSetLinkRequestParams {
  ...
  uint32_t TxID;
  uint32_t RxID;
  //uint8_t nrnodesforuse;  // add by ieong
...
public:
  ...
  //turn on or off csmaca //change by ding
  const static bool csmacamode;

  //send with csmaca
  void Sendcsmaca(void);
  /**
   * Get the type ID.
   *
   ...
   */
  void SetCsmaCa (Ptr<LrWpanCsmaCa> csmaCa); //change by ding
  ...
  // add by ieong
  ////////////////////////////////////////////////////////
  void Sendmatrixtomac (std::vector<std::vector<int> >& matrix);
  
  void scheduling();

  void PrintingTimeslotTable();

  void ConfirmConsecutiveLink();

  void ConsecutiveHandle();

  void calculate_ping6_ASN(double TimeForGenerate);

  void SendConfigurationToMac(double TimeslotLength, int pktsize, double interval, std::vector<std::vector<std::vector<int> > >& DCFL, 
                              int TimeslotForDCFL, std::vector<int>& link, ApplicationContainer apps);

  void TimeslotLength_allocate();

  void TimeslotLength_allocate1();
  ...
  void adaptive_slotframe();

  void e_TSCH_Orch_process();
...
private:
   ...
   * packet.
   */
  uint8_t m_retransmission; //change by ding

  /**
   * The number of CSMA/CA retries used for sending the current packet.
   */
  uint8_t m_numCsmacaRetry; //change by ding
   ...
   */
  TracedCallback<Ptr<const Packet>, uint8_t, uint8_t > m_sentPktTrace; //change by ding
   ...
   */
  Ptr<LrWpanCsmaCa> m_csmaCa; //change by ding
  ...
  void generate_packet(); // add by ieong
  ...
  //std::list<ping6> m_macping6Table;   // add by ieong
