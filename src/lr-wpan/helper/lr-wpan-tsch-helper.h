...
public:
  ...
  * 
  */
  void ConfigureSlotframeAllToPanwithSlotframe(NetDeviceContainer devs, int empty_timeslots, bool bidir, bool bcast, int SlotframeSize);  

  void ConfigureSlotframeAllToPanForSharedLink_1high(NetDeviceContainer devs, int empty_timeslots, bool bidir, bool bcast, int NumberOfSharedLink);
  
  void ConfigureSlotframeAllToPanForSharedLink_RandomTraffic(NetDeviceContainer devs, int empty_timeslots, bool bidir, bool bcast, int NumberOfSharedLink);

  void ConfigureSlotframeAllToPanForTree(NetDeviceContainer devs, int empty_timeslots, bool bidir, bool bcast, int nrnodes); 

  void ConfigureSlotframeAllToPanForTreeBaseOnPacket(NetDeviceContainer devs, int size); 

  void Sendmatrixtohelper(std::vector<std::vector<int> >& matrix, Ptr<NetDevice> dev); 

  void Sendpositiontohelper(std::vector<double>& x, std::vector<double>& y, std::vector<double>& z);

  void generate_packet(); 

  void generate_ranking_matrix();

  void scheduling();  

  void GenerateTrafficBaseOnPacket(NetDeviceContainer devs,int packet_size, double start, double duration, double interval); 

  void SendPacketBaseOnPacket(NetDeviceContainer devs,int packet_size, double interval, double end); 

  void GenerateTrafficBaseOnPacketForETree(NetDeviceContainer devs,int packet_size, double start, double duration, double interval, int nrnodes);

  void SendPacketBaseOnPacketForETree(NetDeviceContainer devs,int packet_size, double interval, double end, int nrnodes);

  void scheduling_ETree(int nrnodes, int slotframe_size);

  void scheduling_TASA(); 

  void matching_TASA(int node_n); 
  
  bool CheckOverDistance(int i, int j);

  int GetActiveTimeslot();

  int GetTotalpacket();

  void GenerateTrafficBaseOnPacketForStar(NetDeviceContainer devs,int packet_size, double start,double end);

  void SendPacketBaseOnPacketForStar(NetDeviceContainer devs,int packet_size, double interval, double end);

  void SendConfigurationToHelper(NetDeviceContainer devs, double TimeslotLength, int pktsize, double interval, ApplicationContainer apps);
  ...
  void SendPacketAndUpdatePing6(Ptr<NetDevice> dev, Address dst, double interval, ApplicationContainer apps, int nrnodes);
  ...
  void GenerateTrafficWithSrc(Ptr<NetDevice> dev, Address source, Address dst, int packet_size, double start, double duration, double interval);
  ...
  void SendPacketWithSrc(Ptr<NetDevice> dev, Address source, Address dst, int packet_size, double interval,double end);
