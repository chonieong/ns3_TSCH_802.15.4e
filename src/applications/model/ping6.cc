...
void Ping6::Send ()
{
  ...
  //std::cout << "1 p->GetSize () : " << p->GetSize () << std::endl;
  p->AddAtEnd (Create<Packet> (m_size - 4));
  //std::cout << "2 p->GetSize () : " << p->GetSize () << std::endl;
  ...
  p->AddHeader (req);
  //std::cout << "3 p->GetSize () : " << p->GetSize () << std::endl;
  ...
  //std::cout << "4 p->GetSize () : " << p->GetSize () << std::endl;


  m_socket->SendTo (p, 0, Inet6SocketAddress (m_peerAddress, 0));
  //std::cout << "5 p->GetSize () : " << p->GetSize () << std::endl;
  ++m_sent;
  //std::cout << "6 p->GetSize () : " << p->GetSize () << std::endl;


  NS_LOG_INFO ("Sent " << p->GetSize () << " bytes to " << m_peerAddress);
  std::cout << "Ping6 is coming, sent " << p->GetSize () << " bytes to " << m_peerAddress << std::endl;
  if (m_sent < m_count)
    {
      ...
      if (Inet6SocketAddress::IsMatchingType (from))
        {
          ...
          switch (type)
            ...
            case Icmpv6Header::ICMPV6_ECHO_REPLY:
              ...
              //std::cout << "rrr p->GetSize () : " << packet->GetSize () << std::endl;
            ...
            case Icmpv6Header::ICMPV6_ERROR_DESTINATION_UNREACHABLE:
              ...
              //std::cout << "rrr p->GetSize () ICMPV6_ERROR_DESTINATION_UNREACHABLE: " << packet->GetSize () << std::endl;
            ...
            case Icmpv6Header::ICMPV6_ERROR_TIME_EXCEEDED:
              ...
              //std::cout << "rrr p->GetSize () Received Time Exceeded from: " << packet->GetSize () << std::endl;
              NS_LOG_INFO ("Received Time Exceeded from " << address.GetIpv6 ());
              break;
            default:
              //std::cout << "rrr error " << std::endl;
              break;
            }
        }
    }
...
uint32_t Ping6::GetSize (void) const
{
  return m_size;
}

void Ping6::SetSize (uint32_t size) 
{
  m_size = size;
}
